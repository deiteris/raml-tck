...
| `Types/Type Expressions/union-with-declaration-facets/` | § Union Type: an instance is valid "if and only if it meets all restrictions associated with at least one of the super types", and "A union type MAY use facets defined by any of its member types as long as all member types in the union accept those facets". The existing `union-with-facets/` covers scalar facets. These fixtures cover the facets that hold declarations: `properties` beside a union of objects and `items` beside a union of arrays, each constraining every member. The invalid fixtures give an example that breaks the added constraint, and `properties` beside a union with a `string` member, which does not accept it. |
| `Types/Type Expressions/union-with-property-enum/` | § Union Type: "If a union type contains a facet with an `enum`, every value of that `enum` MUST meet all restrictions associated with at least one of the super types". The existing enum fixtures put the `enum` on the union itself. Here it is inside a property that each member restates with its own `enum`, so each value fits a different member (`valid.raml`), all values fit one member (`valid-one-member.raml`), or a value fits a member of a nested union (`valid-nested-union.raml`). The invalid fixtures give a value no member admits, and an example outside the `enum`. `invalid-spec-example-default-type.raml` is the section's own example, marked VALID there, which the spec's rules reject: the restated `daysAllowed` has no `type`, so § Determine Default Types makes it `string`, and § Object Type allows a sub-type only to narrow a property's type. The other fixtures therefore give the restated property an explicit `type`. |
